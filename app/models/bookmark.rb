class Bookmark < ActiveRecord::Base
    ...
    def self.most_bookmarked_brewery
        ...
        puts "#{most_brew.brewery.name.split.map(&:capitalize).join(' ')}, in #{most_brew.brewery.city.split.map(&:capitalize).join(' ')}, CA, has the most bookmarks with #{most_brew.cnt} bookmarks."
    end
    ...
    def self.best_brewery
        ...
        puts "#{best_brew.brewery.name.split.map(&:capitalize).join(' ')}, in #{best_brew.brewery.city.split.map(&:capitalize).join(' ')}, CA, is our fans' favorite with a score of #{best_brew.average_rating}."
